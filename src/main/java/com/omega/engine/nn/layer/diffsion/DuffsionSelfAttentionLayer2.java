...
import com.omega.engine.nn.layer.transformer.FastCausalSelfAttentionLayer;
 ...
 *
 */
public class DuffsionSelfAttentionLayer2 extends Layer{
	...
	public DuffsionSelfAttentionLayer2(int embedDim,int headNum,int time,boolean bias,boolean dropout) {
		this.bias = bias;
		this.time = time;
	...
	public DuffsionSelfAttentionLayer2(int embedDim,int headNum,int time,boolean bias,boolean dropout,Network network) {
		this.bias = bias;
...
//		this.batchSize = number / time;
		...
		if(this.preatt == null || this.preatt.number != this.batchSize || this.preatt.width != this.time) {
			...
			this.inputT = Tensor.createTensor(this.inputT, batchSize, time, 1, embedDim, true);
			...
			this.output = Tensor.createTensor(this.output, batchSize, embedDim, 1, time, true);
		}
...
//		this.batchSize = number / time;
	...
	public void initBack() {
		...
		if(this.dvaccum == null){
			...
			this.dqkv = Tensor.createTensor(this.dqkv, batchSize, time, 1, 3 * embedDim, true);
			this.diff = Tensor.createTensor(this.diff, batchSize, input.channel, input.height, input.width, true);
		}else {
//			this.dqkv.clearGPU();
...
//		this.input.showDM();
		this.input.view(batchSize, embedDim, 1, time);
		TensorOP.permute(this.input, inputT, new int[] {0, 3, 2, 1});
		
		this.qkvLinerLayer.forward(inputT);
		...
		Tensor ot = this.oLinerLayer.getOutput();
		
		if(dropout) {
			dropoutLayer2.forward(this.oLinerLayer.getOutput());
			ot = dropoutLayer2.getOutput();
		}
		
		TensorOP.permute(ot, output, new int[] {0, 3, 2, 1});
...
//		GPUOP.getInstance().bmm(CUBLAS_OP_N, CUBLAS_OP_T, dk, time, time, 1.0f, qt.getGpuData(), dk, time * dk, dpreatt.getGpuData(), time, time * time, 0.0f, dkt.getGpuData(), dk, time * dk, batchSize * headNum);
	...
	public void diff() {
		...
		output.view(batchSize, embedDim, 1, time);
		
		TensorOP.permute(delta, output, new int[] {0, 3, 2, 1});
		
		if(dropout) {
			dropoutLayer2.back(output);
			this.oLinerLayer.back(dropoutLayer2.diff, oi);
		}else {
			this.oLinerLayer.back(output, oi);
		}
		...
		diff.view(batchSize, embedDim, 1, time);
		
		TensorOP.permute(this.qkvLinerLayer.diff, diff, new int[] {0, 3, 2, 1});
