 ...
 *
 */
public class FullyLayer2 extends Layer{
	
	private FullyKernel kernel;
	
	public FullyLayer2(int inputNum,int outputNum) {
		this.channel = 1;
		this.height = 1;
		this.width = inputNum;
	...
	public FullyLayer2(int inputNum,int outputNum,boolean hasBias) {
		this.channel = 1;
		this.height = 1;
		this.width = inputNum;
		this.oChannel = channel;
	...
	public FullyLayer2(int inputNum,int outputNum,boolean hasBias,Network network) {
	...
	public FullyLayer2(int inputNum,int outputNum,int time,boolean hasBias,Network network) {
		this.network = network;
		this.channel = 1;
		this.height = 1;
		this.width = inputNum;
	...
	public static FullyLayer2 createRNNCell(int inputNum,int outputNum,int time,boolean hasBias,Network network) {
		return new FullyLayer2(inputNum, outputNum, time, hasBias, network);
	}
...
//		this.bias = new Tensor(1, 1, 1, oWidth, RandomUtils.kaimingUniformBias(x, n), true);
		...
		this.diffW = new Tensor(1, 1, oWidth, width, true);
	}
	
	public void initParamRNNCell() {
		// TODO Auto-generated method stub
		this.weight = new Tensor(1, 1, oWidth, width, RandomUtils.uniformFloat(this.width * this.oWidth, this.oWidth), true);
		this.diffW = new Tensor(1, 1, oWidth, width, true);
		if(hasBias) {
...
//			input.showDMByNumber(0);
			GPUOP.getInstance().multiplyFloat(number, width, oWidth, input.getGpuData(), weight.getGpuData(), output.getGpuData(),
...
//			output.showDMByNumber(0);
	...
	public void output(int batch,int step) {
		...
		if(this.input != null) {
			...
			GPUOP.getInstance().multiplyFloat(batch, width, oWidth, input.getGpuData().withByteOffset(step * batch * input.getOnceSize() * Sizeof.FLOAT),
	...
	public void output(int batch,int inputStep,int step) {
		...
		if(this.input != null) {
			...
			if(inputStep >= 0) {
				...
				GPUOP.getInstance().multiplyFloat(batch, width, oWidth, input.getGpuData().withByteOffset(inputStep * batch * input.getOnceSize() * Sizeof.FLOAT),
						weight.getGpuData(), output.getGpuData().withByteOffset(step * batch * output.getOnceSize() * Sizeof.FLOAT),
						cublasOperation.CUBLAS_OP_N, cublasOperation.CUBLAS_OP_T, 1.0f, 0.0f);
	...
	public void diff() {
		 ...
		 * m = w,k = number,n = ow
		 */
		GPUOP.getInstance().multiplyFloat(this.oWidth, this.width, this.number, delta.getGpuData(), input.getGpuData(), diffW.getGpuData(),
	...
	public void diff(int batch,int step) {
		 ...
		 * m = w,k = number,n = ow
		 */
		GPUOP.getInstance().multiplyFloat(this.oWidth, this.width, batch, delta.getGpuData().withByteOffset(step * batch * delta.getOnceSize() * Sizeof.FLOAT),
				input.getGpuData().withByteOffset(step * batch * input.getOnceSize() * Sizeof.FLOAT), diffW.getGpuData(),
	...
	public void diff(int batch,int inputStep,int step) {
		 ...
		 * m = w,k = number,n = ow
		 */
		GPUOP.getInstance().multiplyFloat(this.oWidth, this.width, batch, delta.getGpuData().withByteOffset(inputStep * batch * delta.getOnceSize() * Sizeof.FLOAT),
				input.getGpuData().withByteOffset(step * batch * input.getOnceSize() * Sizeof.FLOAT), diffW.getGpuData(),
				cublasOperation.CUBLAS_OP_T, cublasOperation.CUBLAS_OP_N, 1.0f, 1.0f);
	...
	public void diff(int batch,int inputStep,int deltaStep,int step) {
		...
		if(step >= 0) {
			 ...
			 */
			GPUOP.getInstance().multiplyFloat(this.oWidth, this.width, batch, delta.getGpuData().withByteOffset(step * batch * delta.getOnceSize() * Sizeof.FLOAT),
					input.getGpuData().withByteOffset(inputStep * batch * input.getOnceSize() * Sizeof.FLOAT), diffW.getGpuData(),
					cublasOperation.CUBLAS_OP_T, cublasOperation.CUBLAS_OP_N, 1.0f, 1.0f);
	...
	public void diff(Tensor input,Tensor diff,int batch,int deltaStep) {
		 ...
		 * m = w,k = number,n = ow
		 */
		GPUOP.getInstance().multiplyFloat(this.oWidth, this.width, batch, delta.getGpuData(),
				input.getGpuData().withByteOffset(deltaStep * batch * input.getOnceSize() * Sizeof.FLOAT), diffW.getGpuData(),
				cublasOperation.CUBLAS_OP_T, cublasOperation.CUBLAS_OP_N, 1.0f, 1.0f);
	...
	public void diffTemp() {
		 ...
		 */
		GPUOP.getInstance().multiplyFloat(this.number, this.width, this.oWidth, delta.getGpuData(), weight.getGpuData(), diff.getGpuData(),
				cublasOperation.CUBLAS_OP_N, cublasOperation.CUBLAS_OP_N, 1.0f, 1.0f);
