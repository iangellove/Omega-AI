...
public class NetworkUtils {
	...
	public static void save(Network nn,String path,String name) {
		...
		for(Layer layer:nn.layerList) {
			...
			case input:
				...
				break;
			case full:
				lc.setWeight(NetworkUtils.toTensor(layer.weight, 1, 1, lc.getWidth(), lc.getoWidth()));
				lc.setBias(NetworkUtils.toTensor(layer.bias, 1, 1, 1, lc.getoWidth()));
				break;
			case conv:
				ConvolutionLayer cl = (ConvolutionLayer) layer;
				lc.setWeight(NetworkUtils.toTensor(cl.kernel, cl.kernelNum, cl.channel, cl.kHeight, cl.kWidth));
				lc.setBias(NetworkUtils.toTensor(layer.bias, 1, 1, 1, cl.kernelNum));
			...
			case bn:
				...
				lc.setGama(bnl.gama);
				lc.setBeta(bnl.beta);
				lc.setRuningMean(bnl.runingMean);
				lc.setRuningVar(bnl.runingVar);
				break;
			default:
				break;
			}
	...
	public static Tensor toTensor(float[][][][] x,int number,int channel,int height,int width) {
		Tensor t = new Tensor(number, channel, height, width);
		t.data = MatrixUtils.transform(x);
		return t;
	}
	...
	public static float[][][][] to4d(Tensor x){
		return MatrixUtils.transform(x.data, x.number, x.channel, x.height, x.width);
	}
	
	public static float[][] to2d(Tensor x){
		return MatrixUtils.transform(x.data, x.height, x.width);
	}
	...
	public static Network loadNetworkConfig(String path) {
		...
		for(int i = 0;i<nc.getLayers().size();i++) {
			...
			case "full":
				...
				l.weight = NetworkUtils.to2d(lc.getWeight());
				l.bias = lc.getBias().data;
				break;
			case "conv":
				...
				conv.kernel = NetworkUtils.to4d(lc.getWeight());
				l.bias = lc.getBias().data;
