...
public class ConvCudnnKernel extends ConvBaseKernel{
	...
	private cudnnFilterDescriptor dkernelDesc;
	...
	public ConvCudnnKernel(Network network,int C,int H,int W,int ko,int kh,int kw,int s,int p) {
		...
		dkernelDesc = new cudnnFilterDescriptor();
		...
		JCudnn.cudnnCreateFilterDescriptor(dkernelDesc);
	...
	public void init(int number) {
		...
		if(this.N != number) {
			...
			JCudnn.cudnnSetFilterNdDescriptor(dkernelDesc, CUDNN_DATA_FLOAT, CUDNN_TENSOR_NCHW, 4, weight);
			...
			JCudnn.cudnnSetTensor4dDescriptor(dyDesc, CUDNN_TENSOR_NCHW, CUDNN_DATA_FLOAT, this.on, this.oc, this.oh, this.ow);
			
			this.fw_algo = getForwardAlgorithm(convAlgorithm, xDesc, kernelDesc, convDesc, yDesc);
			this.bkf_algo = getBKFGO(convDims, xDesc, dyDesc, dkernelDesc, convDesc, yDesc);
			this.bkd_algo = getBKDGO(convDims, dxDesc, dyDesc, kernelDesc, convDesc, yDesc);
	...
	public void dw(Tensor input,Tensor delta,Tensor dKernel) {
		...
		handle(JCudnn.cudnnConvolutionBackwardFilter(CudnnHandleManager.getHandle(), alpha_P, xDesc, input.getGpuData(), dyDesc, delta.getGpuData(), convDesc, bkf_algo,
				this.network.workspace, this.network.workspaceSize, beta_P, dkernelDesc, dKernel.getGpuData()));

	}
	
	public void dx(Tensor delta,Tensor kernel,Tensor diff) {

		handle(JCudnn.cudnnConvolutionBackwardData(CudnnHandleManager.getHandle(), alpha_P, kernelDesc, kernel.getGpuData(), dyDesc, delta.getGpuData(), convDesc, bkd_algo,
				this.network.workspace, this.network.workspaceSize, beta_P, dxDesc, diff.getGpuData()));
...
//		System.out.println("bkf_algo:"+bkf_algo);
		handle(JCudnn.cudnnGetConvolutionBackwardFilterWorkspaceSize(CudnnHandleManager.getHandle(), xDesc, dyDesc, convDesc, dkernelDesc, bkf_algo, sa));

		if(sa[0] > most) {
			most = sa[0];
		}
//		System.out.println("bkd_algo:"+bkd_algo);
		handle(JCudnn.cudnnGetConvolutionBackwardDataWorkspaceSize(CudnnHandleManager.getHandle(), kernelDesc, dyDesc, convDesc, dxDesc, bkd_algo, sa));
