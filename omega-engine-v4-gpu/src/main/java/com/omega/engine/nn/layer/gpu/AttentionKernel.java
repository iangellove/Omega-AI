	...
	public void initFunction() {
		...
		try {
			...
			if(permute_function == null) {
				...
				permute_function = CUDAModules.getFunctionByModule(LibPaths.LIB_PATH+"AttentionKernel.cu", "permute_kernel");
				
			}
			
			if(unpermute_function == null) {

				unpermute_function = CUDAModules.getFunctionByModule(LibPaths.LIB_PATH+"AttentionKernel.cu", "unpermute_kernel");
				
			}
			
			if(permute_backward_function == null) {

				permute_backward_function = CUDAModules.getFunctionByModule(LibPaths.LIB_PATH+"AttentionKernel.cu", "permute_kernel_backward");
				
			}
			
			if(unpermute_backward_function == null) {

				unpermute_backward_function = CUDAModules.getFunctionByModule(LibPaths.LIB_PATH+"AttentionKernel.cu", "unpermute_kernel_backward");
				
			}
			
			if(softmax_forward_function == null) {
//				softmax_forward_function = CUDAModules.getFunctionByModule(LibPaths.LIB_PATH+"AttentionKernel.cu", "softmax_forward_kernel"); 
				softmax_forward_function = CUDAModules.getFunctionByModule(LibPaths.LIB_PATH+"AttentionKernel.cu", "softmax_forward_kernel4");
			}
...
//				softmax_backward_function = CUDAModules.getFunctionByModule(LibPaths.LIB_PATH+"AttentionKernel.cu", "softmax_autoregressive_backward_kernel");
			...
			if(scale_function == null) {
				scale_function = CUDAModules.getFunctionByModule(LibPaths.LIB_PATH+"AttentionKernel.cu", "scale_kernel");
			}
	 ...
	 * @param T
	 */
	public void softmax_forward(Tensor input,Tensor output,int B,int NH,int T) {
...
//		    int grid_size = get_number_of_blocks(B * NH * T * 32, softmax_block_size);
	 ...
	 * @param T
	 */
	public void softmax_backward(Tensor dpreatt,Tensor datt,Tensor att,int B,int T,int C,int NH) {
		...
		try {
	         ...
	         * float* __restrict__ dpreatt, const float* __restrict__ datt,
               const float* __restrict__ att,int B, int T, int C, int NH
	         */ 
