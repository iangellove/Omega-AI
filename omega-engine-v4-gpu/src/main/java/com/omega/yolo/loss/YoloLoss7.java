...
//	            	System.out.println(iou);
	            ...
	            for (int n = 0; n < total; ++n) {
	            	...
	            	if (mask_n_t >= 0 && n != bestIndex && iou_thresh < 1.0f) {
...
//		            	System.out.println("iou:"+iou);
		            	if (iou > iou_thresh) { 
		            		...
		            		int box_index = entryIndex(b, x.width, x.height, mask_n_t, 0);
		            		
		            		float ciou = deltaYoloBox(truth, x, anchors, bestIndex, box_index, i, j, x.width, x.height, stride);
		            		
		            		int obj_index = entryIndex(b, x.width, x.height, mask_n_t, 4);
...
//	        	System.out.println(n_index);
	...
	public static void scal_add_cpu(Tensor x,int N, float ALPHA, float BETA, int index, int INCX){
	    for (int i = 0; i < N; ++i) {
	    	x.data[index + i*INCX] = x.data[index + i*INCX] * ALPHA + BETA;
	    }
	}
