...
public class PowOP extends FunctionOP {
	...
	@Override
	public Tensor forward(Tensor left,double e) throws AutogradException {
	...
	public void backward(Tensor seft) {
		...
		if(seft.getLeftTensor()!=null && seft.getLeftTensor().isRequiresGrad()) {
			 ...
			 */
			double[][][][] thisTensor = MatrixOperation.multiplication(MatrixOperation.pow(seft.getLeftTensor().getData(), seft.getE() - 1), seft.getE());
